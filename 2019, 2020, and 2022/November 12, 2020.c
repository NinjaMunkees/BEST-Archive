#pragma config(Motor,  port2,           LMotor,        tmotorServoContinuousRotation, openLoop, reversed)
#pragma config(Motor,  port3,           RMotor,        tmotorServoContinuousRotation, openLoop)
#pragma config(Motor,  port4,           LwnMotor,      tmotorServoContinuousRotation, openLoop)
#pragma config(Motor,  port5,           PowPow,        tmotorServoContinuousRotation, openLoop)
#pragma config(Motor,  port6,           OptPrime,      tmotorServoStandard, openLoop)
//*!!Code automatically generated by 'ROBOTC' configuration wizard               !!*//

	bool autoA;
	bool autoB;

task GottaGoFast()
{
float left, right;
while (true)
	{
		if(abs(vexRT[Ch3]) < 10 && abs(vexRT[Ch2]) < 10)
		{
			motor[LMotor] = 0;
			motor[RMotor] = 0;
		}
		else
		{
			left = pow(vexRT[Ch3], 3)/pow(127, 2);
			right = pow(vexRT[Ch2], 3)/pow(127, 2);

			if(left > 127)
			{
				left = 65;
			}
			else if(left < -127)
			{
				left = -65;
			}
			if(right > 127)
			{
			 right = 127;
			}
			else if(right < -127)
			{
				right = -127;
			}
			motor[LMotor] = left;
			motor[RMotor] = right;
		} // Watch for wheels
	}
}

task GrassEater()
{
	while (true)
	{
		if (vexRT[Btn6U] == true)
		{
			motor[LwnMotor] = 90;
		}
		else if (vexRT[Btn6D] == true)
		{
			motor[LwnMotor] = -90;
		}
		else
		{
			motor[LwnMotor] = 0;
		}
	}
}

task BlamBlam()
{
	while (true)
	{
		if (vexRT[Btn8R] == true)
		{
			motor[PowPow] = -90;
		}
		else
		{
			motor[PowPow] = 0;
		}
	}
}

task AutoBots()
{
motor[LMotor] = 65;
motor[RMotor] = 127;
wait1Msec(4600);
motor[LMotor] = 0;
motor[RMotor] = 0;
wait1Msec(15);
motor[OptPrime] = 230;
wait1Msec(120);
motor[LMotor] = -65;
motor[RMotor] = -127;
wait1Msec(350);
motor[LMotor] = 0;
motor[RMotor] = 0;
startTask(GottaGoFast);
}

task RollOut()
{
motor[LMotor] = 65;
motor[RMotor] = 127;
wait1Msec(1800);
motor[LMotor] = 0;
motor[RMotor] = 0;
wait1Msec(15);
motor[OptPrime] = 230;
wait1Msec(120);
motor[LMotor] = -65;
motor[RMotor] = -127;
wait1Msec(350);
motor[LMotor] = 0;
motor[RMotor] = 0;
startTask(GottaGoFast);
}

task main()
{
	startTask(GottaGoFast);
	startTask(GrassEater);
	startTask(BlamBlam);

	autoA = false;
	autoB = false;

	while(true)
	{
		wait1Msec(10);

		if(vexRT[Btn7U] == true && autoA == false)
		{
			autoA = true;	//Raise the autoA flag
			stopTask(GottaGoFast);	//To keep the drive task from interfeering with the robot
			startTask(AutoBots);
		}
		//Interupting autonomous mode & restart remote control driving
		else if(autoA == true && (abs(vexRT[Ch2]) > 40 || abs(vexRT[Ch3]) > 40))
		{
			autoA = false;	//Clear the autoA flag
			stopTask(AutoBots);
			startTask(GottaGoFast);
			motor[OptPrime] = 0;
		}
		if(vexRT[Btn7D] == true && autoB == false)
		{
			autoB = true;	//Raise the autoB flag
			stopTask(GottaGoFast);	//To keep the drive task from interfeering with the robot
			startTask(RollOut);
		}
		//Interupting autonomous mode & restart remote control driving
		else if(autoB == true && (abs(vexRT[Ch2]) > 40 || abs(vexRT[Ch3]) > 40))
		{
			autoB = false;	//Clear the autoB flag
			stopTask(RollOut);
			startTask(GottaGoFast);
			motor[OptPrime] = 0;
		}
	}
}
